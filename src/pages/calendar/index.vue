  ...
  <view class="bg-[#f8faff] min-h-screen overflow-hidden">
    ...
    <view class="pt-8 pb-2 flex flex-col items-center">
      ...
      <view class="flex items-center mt-1">
        <view
          class="w-10 h-10 rounded-full bg-gradient-to-r from-green-400 to-emerald-500 mr-1 animate-pulse"
        />
        <text class="text-xs text-gray-400"
          >共
          <text class="font-bold text-blue-600">{{ recordCount }}</text>
          条记录</text
        >
      </view>
    </view>
    ...
    <!-- 日历卡片 -->
    <view
      class="calendar-card bg-white rounded-2xl shadow-lg mx-4 mt-4 p-4 relative overflow-hidden"
    >
      ...
      <view class="flex items-center justify-between mb-8">
        ...
        <view class="flex-1 flex justify-center">
          <text class="text-xl font-bold text-center calendar-title"
            >{{ year }}年 {{ month + 1 }}月</text
          >
        </view>
      ...
      <view>
        <view
          v-for="(row, rIdx) in calendarRows"
          :key="rIdx"
          class="grid grid-cols-7 mb-1 gap-2"
        >
          <view
    ...
    <!-- 图例 -->
    <view
      class="legend-card bg-white rounded-2xl shadow-lg mx-4 mt-6 p-4 flex items-center justify-center gap-8 relative"
    >
      <div class="shimmer-bg"></div>
      ...
      <view class="p-5">
        ...
        <view v-if="selectedRecord" class="record-details">
          <view class="flex items-center mb-3">
            <view
              class="detail-icon w-8 h-8 rounded-full mr-3"
              :class="[
          ...
          <view class="flex justify-between detail-item">
            ...
            <text class="font-bold text-green-600"
              >{{ selectedRecord.calorie }} 卡</text
            >
          </view>
        </view>
        <view v-else class="flex flex-col items-center justify-center py-6">
          <text
            class="i-mdi:calendar-blank text-6xl text-gray-200 mb-3 empty-icon"
          ></text>
...
const calendarRows = computed(() => {
  ...
  for (let d = 1; d <= days; d++) {
    const dateStr = `${year.value}-${String(month.value + 1).padStart(
      2,
      "0"
...
function selectDay(cell: any) {
  if (cell && cell.isCurrentMonth) {
    ...
    selectedRecord.value =
      records.value.find((r) => r.date === dateStr) || null;
